    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link" href="/post/list">포럼</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="/form/contact">웹폼</a>
          </li>
        ...
        <a href="/member/mypage" v-if="output?.user.auth_flg" class="mx-1 btn btn-outline-dark">마이페이지</a>
        <button v-if="output?.user.auth_flg" class="mx-1 btn btn-outline-dark h-btn-sign-out" data-return-url="/" type="button">로그아웃</button>
        <a href="/member/login" v-if="output && !output.user.auth_flg" class="mx-1 btn btn-outline-dark">로그인</a>
        <a href="/member/join" v-if="output && !output.user.auth_flg" class="mx-1 btn btn-outline-dark">회원가입</a>
      </div>
    </div>
  </nav>
</template>
